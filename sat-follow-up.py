...
def follow_up_test(fup_test, fup_test_exp_sat):
    ...
    if fup_test_proc.returncode != 0:
        print("%s: Abnormal termination (return code %d): %s, expect: %s" % \
            (fup_test, fup_test_proc.returncode, str(fup_test_proc_output[1].decode("utf-8")), fup_test_exp_sat))
...
for input_test in input_tests:
    ...
    for fup_test in follow_up_tests:
        ...
        if not fup_test_exp_sat:
            ...
            continue
        try:
            follow_up_test(fup_test, fup_test_exp_sat)
        except (timeout_decorator.TimeoutError):
            print(fup_test + " UNKNOWN expected:" + fup_test_exp_sat)
            continue
